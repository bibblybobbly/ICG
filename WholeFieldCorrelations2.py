...
e1=j.field('e1')
e2=j.field('e2')
flux=j.field('mean_flux')
radius=j.field('radius')
...
for g in range(1, len(infile)):
	...
	e1add=ja.field('e1')
	e2add=ja.field('e2')
	fluxadd=ja.field('mean_flux')
	radiusadd=ja.field('radius')
	...
	fluxs=np.append(flux, fluxadd, axis=0)
	radiuss=np.append(radius, radiusadd, axis=0)
...
print('flux variance', np.var(flux))
print('Average flux', np.average(flux))
print('median flux', np.median(flux))


#mpl.scatter(x, y)
#mpl.title('Whole field')
#mpl.savefig('WholeField/WholeFieldPoint.png')
#mpl.close()


mpl.hist(flux, bins=100)
mpl.savefig('WholeField/Flux histogram 100bins.png')
mpl.close()

mpl.hist(flux, bins=1000)
mpl.savefig('WholeField/Flux histogram 1000bins.png')
mpl.close()

mpl.hist(np.log(flux), bins=1000)
mpl.savefig('WholeField/Flux histogram log 1000bins.png')
mpl.close()

mpl.scatter(radius, flux)
mpl.title('Flux as a function of radius')
mpl.savefig('WholeField/RadFlux.png')
mpl.close()


mpl.hist(np.divide(flux, radius), bins=1000)
mpl.title('Histogram of flux/rad')
mpl.savefig('WholeField/FluxRad hist 1000bins.png')
mpl.close()

mpl.scatter(radius, np.log(flux))
mpl.title('Log(flux) as a function of radius')
mpl.savefig('WholeField/RadLogFlux.png')
mpl.close()
...
fluxfieldacut=fluxfielda[0:xfieldnuma-1]
...
UsefulRatio=64/(60*60)
binzb1=binzb1/UsefulRatio
...
xscale=stepx*60 #steps in x scale bins in terms of arcminutes, ie deg/60=arcmin
...
fluxfielda=np.zeros((binz+1, binz+1))
...
for df in range(0, reps):
	...
	fluxfielda[xval, yval]=fluxfielda[xval, yval]+fluxfieldacut[df]
...
FluxBox=[0]*((binz+1)**2)
CountBox=[0]*((binz+1)**2)
SkyDens=np.zeros((binz+1, binz+1))
PerArcMinCount=[0]*((binz+1)**2)
...
for d in range(0, binz+1):
	for f in range(0, binz+1):
		...
		e2ANorm[d,f]=e2fielda[d,f]/densmapa[d,f]
		Avals[j]=densmapa[d,f]
		FluxBox[j]=fluxfielda[d,f]
		CountBox[j]=densmapa[d,f]
		SkyDens[d,f]=np.divide(densmapa[d,f],(xscale*yscale)) #in per arcmin**2
		PerArcMinCount[j]=SkyDens[d,f]
		j=j+1
			

mpl.hist(np.log(PerArcMinCount), bins=1000)
mpl.title('Galaxies per arcminute')
mpl.savefig('WholeField/GalArcminlog.png')
mpl.close()
...
MassFluxRat=np.divide(MassBox, FluxBox)
#KappaMassRat=np.divide(MassBox,Avals)
#MassRadRat=np.divide(MassBox, radius)
#FluxRadRat=np.divide(FluxBox, radius)
logFluxRat=np.divide(np.log(flux), radius)

mpl.hist(logFluxRat, bins=1000)
mpl.xlabel('log(Flux)/radius')
mpl.savefig('WholeField/logF rad.png')

#mpl.hist(FluxRadRat, bins=1000)
#mpl.xlabel('Flux/radius')
#mpl.savefig('WholeField/FluxRad hist.png')

mpl.scatter(Avals, MassBox)
mpl.xlabel('Count')
mpl.ylabel('Kappa')
mpl.savefig('WholeField/CountvsMass.png')
mpl.close()

mpl.hist(np.divide(Avals, MassBox), bins=1000)
mpl.xlabel('Count/Kappa')
mpl.savefig('CountKappaHist.png')
mpl.close()


#mpl.hist(KappaMassRat, bins=1000)
#mpl.xlabel('Kappa/Mass')
#mpl.savefig('WholeField/KappaMass.png')

mpl.hist(MassFluxRat, bins=1000)
mpl.xlabel('Mass/Flux')
mpl.savefig('WholeField/KappaMass.png')
...
mpl.colorbar()
pylab.savefig('WholeField/MassMaps/NoNoise/KappaMap 010 boxmin4.png')
mpl.close()

mpl.contourf(np.real(kappa), vmin=0.2)
mpl.colorbar()
pylab.savefig('WholeField/MassMaps/NoNoise/KappaMap 020 boxmin4.png')
mpl.close()

mpl.contourf(np.real(kappa), vmin=0.05)
mpl.colorbar()
pylab.savefig('WholeField/MassMaps/NoNoise/KappaMap 005 boxmin4.png')
mpl.close()

mpl.contourf(np.real(kappa), vmin=0.15)
mpl.colorbar()
pylab.savefig('WholeField/MassMaps/NoNoise/KappaMap 015 boxmin4.png')
mpl.close()

mpl.scatter(MassBox, FluxBox)
mpl.xlabel('mass in box')
mpl.ylabel('Flux in box')
pylab.savefig('WholeField/MassvsFlux.png')
mpl.close()
...
for xlimi in range(width, binz+width, width):
	for ylimi in range(width,binz+width, width):
		...
		if cutoff<= cutoffm:
			...
			pylab.savefig('WholeField/FieldASectionat X'+str(xlimi)+' and Y'+str(ylimi)+'f'+str(suf)+'.png') #
		...
		mpl.xlim((xlimi-width, xlimi))
		mpl.ylim((ylimi-width, ylimi))
		pylab.savefig('WholeField/MassMaps/NoNoise/'+str(xlimi)+' '+str(ylimi)+' Shear on Kappa Cyan 010 cut boxmin4 .png')
		mpl.close()
...
mpl.colorbar()
pylab.savefig('WholeField/MassMaps/Noisy/KappaMap A noise bm4.png')
mpl.close()

mpl.imshow(np.real(kappa))
pylab.savefig('WholeField/MassMaps/Noisy/imshow kappa A with noise bm4.png')
mpl.close()

mpl.contourf(np.real(kappa), vmin=0.1)
mpl.colorbar()
pylab.savefig('WholeField/MassMaps/Noisy/KappaMap 010 bm4.png')
mpl.close()

mpl.contourf(np.real(kappa), vmin=0.2)
mpl.colorbar()
pylab.savefig('WholeField/MassMaps/Noisy/KappaMap 020 bm4.png')
mpl.close()

mpl.contourf(np.real(kappa), vmin=0.05)
mpl.colorbar()
pylab.savefig('WholeField/MassMaps/Noisy/KappaMap 005.png')
mpl.close()

mpl.contourf(np.real(kappa), vmin=0.15)
mpl.colorbar()
pylab.savefig('WholeField/MassMaps/Noisy/KappaMap 015 bm4.png')
mpl.close()
...
for xlimi in range(width, binz+width, width):
	for ylimi in range(width,binz+width, width):
		...
		if cutoff<= cutoffm:
			...
			mpl.xlim((xlimi-width, xlimi))
			mpl.title('Field A section')
			pylab.savefig('WholeField/MassMaps/Noisy/FieldASectionat X'+str(xlimi)+' and Y'+str(ylimi)+'f'+str(suf)+'bm4 .png') #
			print('Produced Quiverf, Field D, xlim= '+str(xlimi)+' and ylim='+str(ylimi)+'')
		...
		mpl.xlim((xlimi-width, xlimi))
		mpl.ylim((ylimi-width, ylimi))
		pylab.savefig('WholeField/MassMaps/Noisy/'+str(xlimi)+' '+str(ylimi)+' Shear on Kappa Cyan 010 cut bm4 .png')
		mpl.close()

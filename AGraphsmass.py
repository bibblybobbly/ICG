...
gamma2=e2/2
print(gamma1[1])
gamma1_2=np.square(gamma1)
print(gamma1[1])
gamma2_2=np.square(gamma2)
gamma_2=gamma1_2+gamma2_2
gamma=np.sqrt(gamma_2)
angles2=np.divide(gamma2, gamma) # equal to e1/e=cos2theta
theta2=np.arcsin(angles2)
theta=theta2/2
...
for o in range(0, len(x)-1):
	if y[o] <= -40:                 #selecting for Field A - ie that with RA below 12
		...
		#np.append(xfielda, x[o], axis=0)
		#np.append(yfielda, y[o], axis=0)
		#np.append(e1fielda, e1[o], axis=0)
		#np.append(e2fielda, e2[o], axis=0)
		#np.append(gammafielda, gamma[o], axis=0)
		yfielda[xfieldnuma]=y[o]
		e1fielda[xfieldnuma]=e1[o]
		e2fielda[xfieldnuma]=e2[o]
		#gammafielda[xfieldnuma]=gamma[o]
		#thetafielda[xfieldnuma]=theta[o]
...
for d in range(0, binz+1):
	for f in range(0, binz+1):
		if e1ANorm[d,f] != 0:
			thetaA[d,f]=np.arctan(np.divide(e1ANorm[d,f], e2ANorm[d,f]))
...
for xlimi in range(width, binzd+width, width):
	for ylimi in range(width,binzd+width, width):
		suf=0
		...
		if cutoff<= cutoffm:
			...
			pylab.savefig('AGraphs/FieldASectionat X'+str(xlimi)+' and Y'+str(ylimi)+'f'+str(suf)+'.png') #
